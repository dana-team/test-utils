// Package testutils is taken from HNC and it's to be used to do testing for K8S controllers
// see https://github.com/kubernetes-sigs/hierarchical-namespaces/blob/master/pkg/testutils/testutils.go
package testutils
...
func writeTempFile(cxt string) string {
	f, err := ioutil.TempFile(os.TempDir(), "-test-*.yaml")
...
func (s silencer) Match(actual interface{}) (bool, error) {
	diffs := actual.(string)
	return diffs == "", nil
}
func (s silencer) FailureMessage(actual interface{}) string {
	return actual.(string)
}
func (s silencer) NegatedFailureMessage(actual interface{}) string {
